...
import { X, Settings, Trophy, Wallet, Share2, ShieldCheck, Zap, RotateCcw, Mic, LayoutGrid, Sparkles, Lock, Unlock } from 'lucide-react';
...
interface RoomToolsModalProps {
  ...
  isAdmin: boolean;
  roomMicsLocked?: boolean;
}

const RoomToolsModal: React.FC<RoomToolsModalProps> = ({ isOpen, onClose, onAction, isAdmin, roomMicsLocked }) => {
  ...
  const allTools = [
    ...
    { id: 'open_mics', label: roomMicsLocked ? 'فتح المايكات' : 'قفل المايكات', icon: roomMicsLocked ? Unlock : Lock, color: roomMicsLocked ? 'bg-emerald-600' : 'bg-indigo-500' },
  ];

  // تصفية الأدوات: الأدمن (مالك أو مشرف) يرى الكل، المستخدم العادي يرى صندوق الحظ والترتيب فقط
  const tools = isAdmin 
    ? allTools 
        ...
        <div className="flex justify-between items-center mb-8">
           <div className="flex items-center gap-2">
              ...
              <h3 className="text-lg font-black text-white text-right">
                {isAdmin ? 'امتيازات الإدارة' : 'خيارات الغرفة'}
              </h3>
           </div>
