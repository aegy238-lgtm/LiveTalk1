...
const UserProfileSheet: React.FC<UserProfileSheetProps> = ({ user: initialUser, onClose, isCurrentUser, onAction, currentUser, allUsers = [], currentRoom, onShowRoomRank }) => {
  ...
  const handleAdminAction = async (action: 'toggleMod' | 'kickMic' | 'kickRoom' | 'resetUserCharm' | 'breakCP' | 'breakFriend') => {
    ...
    if (action === 'breakCP') {
      if (confirm('هل تريد إنهاء ارتباط الـ CP؟')) {
    ...
    if (action === 'breakFriend') {
      if (confirm('هل تريد إنهاء علاقة الصداقة؟')) {
        await updateDoc(doc(db, 'users', user.id), { friendPartner: null });
        if (user.friendPartner?.id) {
           await updateDoc(doc(db, 'users', user.friendPartner.id), { friendPartner: null });
        }
        alert('تم إنهاء علاقة الصداقة بنجاح');
        setShowAdminMenu(false);
      }
      return;
    }
                    ...
                    >
                      ...
                      {user.cpPartner && (
                        <button onClick={() => handleAdminAction('breakCP')} className="w-full p-3 flex items-center gap-3 hover:bg-white/5 border-b border-white/5 text-right text-pink-500" dir="rtl">
                           <Heart size={16} />
                           <span className="text-xs font-bold">فك ارتباط CP</span>
                        </button>
                      )}
                      {user.friendPartner && (
          ...
          <div className="flex flex-col gap-2 mt-4">
            {user.cpPartner && (
              <motion.div 
                ...
                className="bg-gradient-to-r from-pink-600/10 via-purple-600/20 to-pink-600/10 rounded-2xl border border-pink-500/20 p-2 flex items-center justify-center gap-3 relative overflow-hidden"
              >
                 ...
                 <div className="flex items-center gap-2 relative z-10">
                    ...
                    <div className="flex flex-col items-center">
                       ...
                       <span className="text-[7px] font-black text-white/50 uppercase mt-0.5 tracking-tighter">Sweet Couple</span>
                    </div>
                    <div className="relative">
            ...
            {user.friendPartner && (
              <motion.div 
                initial={{ scale: 0.8, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                className="bg-gradient-to-r from-blue-600/10 via-indigo-600/20 to-blue-600/10 rounded-2xl border border-blue-500/20 p-2 flex items-center justify-center gap-3 relative overflow-hidden"
              >
                 <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/stardust.png')] opacity-30"></div>
                 <div className="flex items-center gap-2 relative z-10">
                    <div className="relative">
                    ...
                    <div className="flex flex-col items-center">
                       ...
                       <span className="text-[7px] font-black text-white/50 uppercase mt-0.5 tracking-tighter">Best Friends</span>
                    </div>
                    <div className="relative">
                       <img src={user.friendPartner.avatar} className="w-10 h-10 rounded-full border border-white/20 object-cover" />
                       <div className="absolute -bottom-1 -left-1 text-[10px]">🛡️</div>
                    </div>
                 </div>
              </motion.div>
            )}
