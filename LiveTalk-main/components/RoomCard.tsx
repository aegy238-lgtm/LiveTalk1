...
const RoomCard: React.FC<RoomCardProps> = ({ room, onClick }) => {
  ...
  const idDisplay = useMemo(() => {
     // استخدام الـ ID المخصص القصير الموجود في بيانات المتحدث أو في بيانات الغرفة
     const idValue = host?.customId || room.hostCustomId || host?.id || room.hostId;
     
     if (!idValue) return <span className="text-slate-500 text-[8px]">ID: ---</span>;
     
     const idColor = (host as any)?.idColor || '#fbbf24';
     const isSpecial = host?.isSpecialId || false;
  ...
  }, [host, room.hostCustomId, room.hostId]);
